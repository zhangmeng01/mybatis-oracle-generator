...
<mapper namespace="${package}.dao.${module}.I${className?cap_first}Dao" >
  ...
  <sql id="whereSql">
	...
	<select id="getByIds" resultMap="BaseResultMap">
	     ...
	     <foreach collection="array" item="item" index="index"  open="(" close=")" separator=",">
          ...
          order by order by field(${keyColumn.dbName},
             <foreach collection="array" item="item" index="index"  open="(" close=")" separator=",">
               ${"#{"?html}${"item}"?html}
             </foreach>
          )
	...
	<select id="findList"  parameterType="${package}.entity.${module}.${className?cap_first}" resultMap="BaseResultMap">
      select <include refid="baseColumns"/> from ${tableName} where 1=1 <include refid="whereSql"/>
	        <if test="startCreateTime != null and endCreateTime!=null ">
                and create_time between ${"#"?html}{startCreateTime} and ${"#"?html}{endCreateTime}
            </if>
             <if test="startUpdateTime != null and endUpdateTime!=null ">
                 and update_time between ${"#"?html}{startUpdateTime} and ${"#"?html}{endUpdateTime}
            </if>
            <if test="startDeleteTime != null and endDeleteTime!=null ">
                 and delete_time between ${"#"?html}{startDeleteTime} and ${"#"?html}{endDeleteTime}
            </if>
            <if test="orderBy != null and orderBy!='' ">
                order by ${"$"?html}{orderBy}
            </if>
	</select>

	 <select id="getCount"  parameterType="${package}.entity.${module}.${className?cap_first}" resultType="java.lang.Integer">
	...
	<select id="findEntityWrapper"  parameterType="com.qitoon.framework.utils.sqlPlus.EntityWrapper" resultMap="BaseResultMap">
	     select <include refid="baseColumns"/> from ${tableName} where 1=1 ${ew.getSqlSegment()}
	</select>
    ...
    <update id="updateById" parameterType="${package}.entity.${module}.${className?cap_first}">
	 ...
	 <update id="updateByIds" parameterType="${package}.entity.${module}.${className?cap_first}">
          update ${tableName}  set
	     <trim suffixOverrides="," >
		    <#list columns as column>
		       <#if column.columnName!=primaryKeyName>
				  <if test="entity.${column.columnName} != null">
                     ${column.dbName} = ${"#{"?html}${"entity."?html}${column.columnName}${"}"?html},
				  </if>
			   </#if>
		    </#list>
		  </trim>
